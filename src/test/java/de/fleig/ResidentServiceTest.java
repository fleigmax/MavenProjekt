...
import de.fleig.informatik.meldeauskunft.domain.*;
import de.fleig.informatik.meldeauskunft.repository.*;
import de.fleig.informatik.meldeauskunft.service.*;
...
public class ResidentServiceTest {
	...
	public void testGetFilteredResidentsList2()
	{
		...
		r.setStreet("Musterstra�e");
		r.setCity("Musterstadt");
		r.setDateOfBirth(new Date());
		list.add(r);
		list.add(new Resident("Martina", "Mustermann", "Albertstra�e", "Musterdorf", new Date()));
		list.add(new Resident("Emil", "Dick", "Waldweg", "Musterdorf", new Date()));
		ResidentRepository stub = new ResidentRepositoryStub(list);
		
		ResidentService rs = new BaseResidentService();
		((BaseResidentService) rs).setResidentRepository(stub);
		List<Resident> result = rs.getFilteredResidentsList(new Resident("Martina", "Mustermann", "Albertstra�e", "Musterdorf", new Date()));
		
		assertEquals(1, result.size());
		assertEquals("Martina", result.get(0).getGivenName());
		assertEquals("Mustermann", result.get(0).getFamilyName());
		assertEquals("Albertstra�e", result.get(0).getStreet());
	...
	{
		List<Resident> list = new LinkedList<Resident>();
		list.add(new Resident("Max", "Mustermann", "Musterstra�e", "Musterstadt", new Date()));
		list.add(new Resident("Martina", "Mustermann", "Albertstra�e", "Musterdorf", new Date()));
		list.add(new Resident("Emil", "Dick", "Waldweg", "Musterdorf", new Date()));
		ResidentRepository stub = new ResidentRepositoryStub(list);
		...
		ResidentService rs = new BaseResidentService();
		((BaseResidentService) rs).setResidentRepository(stub);
		List<Resident> result = rs.getFilteredResidentsList(new Resident("Ma*", "Mu*", "*stra�e", "Muster*", new Date()));
		
		assertEquals(2, result.size());

		assertEquals("Max", result.get(0).getGivenName());
		assertEquals("Mustermann", result.get(0).getFamilyName());
		assertEquals("Musterstra�e", result.get(0).getStreet());
		...
		assertEquals("Albertstra�e", result.get(1).getStreet());
	...
	public void testGetUniqueResident1()
	{
		...
		try {
			Resident result = rs.getUniqueResident(new Resident("Martina", "Mustermann", "Albertstra�e", "Musterdorf", null));
	...
	public void testGetUniqueResident3()
	{
		...
		try {
			...
			assertEquals("Musterstra�e", result.getStreet());
